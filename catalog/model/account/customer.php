...
class ModelAccountCustomer extends Model {
	public function addCustomer($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET customer_group_id = '" . (int)$customer_group_id . "', store_id = '" . (int)$this->config->get('config_store_id') . "', language_id = '" . (int)$this->config->get('config_language_id') . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']['account']) ? json_encode($data['custom_field']['account']) : '') . "', salt = '" . $this->db->escape($salt = token(9)) . "', password = '" . $this->db->escape(sha1($salt . sha1($salt . sha1($data['password'])))) . "', newsletter = '" . (isset($data['newsletter']) ? (int)$data['newsletter'] : 0) . "', ip = '" . $this->db->escape($this->request->server['REMOTE_ADDR']) . "', status = '" . ((int)!$customer_group_info['approval'] * (int)!$customer_group_info['email_approval']) . "', date_added = NOW()");

		$customer_id = $this->db->getLastId();
		
		if ($customer_group_info['approval']) {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "customer_approval` SET customer_id = '" . (int)$customer_id . "', approval_code = '0', type = 'customer', date_added = NOW()");			
		} else if ($customer_group_info['email_approval']) {			
			$approval_code = md5($data['email'] . time());			
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer_approval SET customer_id = '" . (int)$customer_id . "', approval_code = '" . $approval_code . "', type = 'customer', date_added = NOW()");
		}
	...
	public function getEmailApprovalCode($customer_id) {
		$query = $this->db->query("SELECT approval_code FROM " . DB_PREFIX . "customer_approval WHERE customer_id='" . $customer_id . "'");
			
		return $query->row['approval_code'];
	}
	
	public function deleteEmailApprovalCode($customer_id){
		$query = $this->db->query("DELETE FROM " . DB_PREFIX . "customer_approval WHERE customer_id='" . $customer_id . "'");	
	}

	public function checkEmailApproved($customer_id){
		$query = $this->db->query("SELECT status FROM " . DB_PREFIX . "customer WHERE customer_id='" . $customer_id . "'");

		return $query->row['status'];
	}

	public function updateEmailApproved($customer_id){
