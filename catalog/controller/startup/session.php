...
class ControllerStartupSession extends Controller {
	public function index() {
		...
		if (isset($this->request->get['route']) && substr($this->request->get['route'], 0, 4) == 'api/' && substr($this->request->get['route'], 0, 9) != 'api/react') {
...
//		} else if (isset($this->request->get['route']) && substr($this->request->get['route'], 0, 9) == 'api/react' && isset($_SERVER['HTTP_ORIGIN']) && $_SERVER['HTTP_ORIGIN'] == 'http://localhost:3000') {		
...
//			setcookie($this->config->get('session_name'), $this->session->getId(), time() + 60 * 60 * 24 * 30, ini_get('session.cookie_path'), ini_get('session.cookie_domain'));		
//		
